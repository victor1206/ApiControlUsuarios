...
namespace SysSeguridad.UIMVC.Controllers
{
    ...
    public class UsuarioController : Controller
    {
        ...
        public async Task<IActionResult> Delete(Usuario pUsuario)
        {
            ...
            usuario.Rol = await rolBL.ObtenerPorIdAsync(new Rol { Id = usuario.Id });
            ViewBag.Error = "";
            return View(usuario);
        }
        ...
        public async Task<IActionResult> Login(Usuario pUsuario, string pReturnURL = null)
        {
            try
            {
                ...
                {
                    usuario.Rol = await rolBL.ObtenerPorIdAsync(new Rol { Id = usuario.IdRol });
                    var claims = new[] { new Claim(ClaimTypes.Name, pUsuario.Login),
                    ...
                    throw new Exception("Credeciales Incorrectas");
        ...
        {
            var usuarios = await usuarioBL.BuscarAsync(new Usuario { Login = User.Identity.Name, Top_Aux = 1, Estatus = 1 });
